...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(r1,r2,exp) => case eval_prog(exp, env) of
                                 NoPoints => NoPoints
                               | Point(x,y) => Point(x+r1,y+r2)
                               | Line(m,b) => Line(m, b+r2-m*r1)
                               | VerticalLine x => VerticalLine(x+r1)
                               | LineSegment(x1,y1,x2,y2) => LineSegment(x1+r1,y1+r2,x2+r1,y2+r2)
...
fun preprocess_prog e = 
  case e of
     ...
     | Shift(r1,r2,exp) => Shift(r1,r2,preprocess_prog(exp))
     | _ => e
     *)
fun preprocess_prog e =
  case e of 
