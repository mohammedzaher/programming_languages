...
(define (racketlist->mupllist rklist)
  (if (null? rklist)
      (aunit)
      (apair (car rklist) (racketlist->mupllist (cdr rklist)))))
...
(define (eval-under-env e env)
  (cond [(var? e) 
        ...
        [(call? e) (let ([closre (eval-under-env (call-funexp e) env)])
                     (if (closure? closre)
                         (let ([f (closure-fun closre)]
                               [v2 (eval-under-env (call-actual e) env)])
                             (if (fun-nameopt f) 
                                 (eval-under-env (fun-body f) (cons (cons (fun-nameopt f) closre)
                                                                     (cons (cons (fun-formal f) v2)
                                                                           (closure-env closre))))
                                 (eval-under-env (fun-body f) (cons (cons (fun-formal f) v2)
                                                                     (closure-env closre)))))
...
(define mupl-map
  (fun #f "f"
       (fun "map" "list" (ifaunit (var "list") (var "list")
                             (apair (call (var "f" ) (fst (var "list"))) (call (var "map") (snd (var "list"))))))))
                      
(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun #f "int"
             (fun "f" "list"
                  (call (call (var "map") (fun #f "i" (add (var "i") (var "int")))) (var "list"))))))
